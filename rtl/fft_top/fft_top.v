...
module fft_top (
    ...
    input              tb_data_cen,
    input              tb_data_wen,
    input      [9:0]   tb_data_addr,     // linear 10-bit address
    input      [31:0]  tb_data_din,
    output     [31:0]  tb_data_dout,

    // Testbench twiddle-SRAM port
    input              tb_tw_cen,
    input              tb_tw_wen,
    input      [8:0]   tb_tw_addr,
    input      [31:0]  tb_tw_din,
    output     [31:0]  tb_tw_dout,

    // Bit-reverse helper (combinational): for testbench to use when loading
    input      [9:0]   tb_linear_addr,   // testbench-supplied linear index
    output     [9:0]   tb_bitrev_addr    // its 10-bit bit-reverse
);
    ...
    wire [31:0] core_data_din;

    wire        core_tw_cen;
    wire [8:0]  core_tw_addr;
    ...
    // Controller <-> core
    wire        core_start;
    wire [31:0] core_x0, core_x1, core_w;
    wire [31:0] core_x0_out, core_x1_out;
    wire        core_done;
