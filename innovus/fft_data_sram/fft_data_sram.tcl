...
set core_width 596
set core_height [expr 66*$std_cell_height]
...
set u_sram_x 5
set u_sram_y 30
placeInstance u_sram $u_sram_x $u_sram_y R0 -fixed
checkPlace
redraw
fit

set sram_wrapper_width 620
set sram_wrapper_height 450
createPlaceBlockage -box [expr $u_sram_x-3]  [expr $u_sram_y-3]  [expr $u_sram_x+$sram_wrapper_width+3]  [expr $u_sram_y+$sram_wrapper_height+3] 
redraw
fit
##################################################
...
addRing -nets {VDD VSS} -type core_rings -layer {top M7 bottom M7 left M8 right M8} -width $ring_top_width \
...
addStripe -block_ring_top_layer_limit M8 \
	  -block_ring_bottom_layer_limit M8 \
...
update_constraint_mode -name typical_constraint -sdc_file "../../dc/$design_name/$design_name.syn.sdc"
...
#write_lef_abstract "$design_name.lef" -5.7 -PgpinLayers 6 -specifyTopLayer 7 -stripePin
write_lef_abstract "$design_name.lef" \
    -5.7 \
    -PgpinLayers {M7 M8} \
    -specifyTopLayer M8 \
    -stripePin
